 ...
 *
 * Скрипт Google Ads который выгружает подробный отчет по поисковым запросам.
 *
 *
 * Скрипт был разработан brainlabsdigital.com
 *
...
function main() {
    ...
    // Нужно поставить релевантный период анализа
    // Формат yyyy-mm-dd

    var currencySymbol = AdWordsApp.currentAccount().getCurrencyCode();
    Logger.log(AdWordsApp.currentAccount().getCurrencyCode());


    var campaignNameContains = "";
    // Использовать данную опцию, если нужен анализ только в определенных кампаниях
    // Нужно вписать слова, которые содержатся, только в целевых кампаниях например 'Brand' or 'Shopping'.
    // Оставить пустым, если нужен анализ по всему аккаунту

    var campaignNameDoesNotContain = "";
    // Использовать данную опцию, если нужно исключить кампанию
    // Нужно вписать слова, которые содержатся, только в целевых кампаниях например 'Brand' or 'Shopping'.
    // Оставить пустым, если нужен анализ по всему аккаунту

    var ignorePausedCampaigns = true;
    // Оставить true, если нужен анализ только по включенным кампаниям
    // Поставить false, если также нужен анализ по оставновленным кампаниям

    var ignorePausedAdGroups = true;
    // Поставить true, если нужен анализ только по включенным группам
    // Поставить false, если также нужен анализ по оставновленным группам

    var checkNegatives = true;
    // Оставть true, для запросов, которые будут исключены с помощью минус-слов

    // Url целевого Google Sheet, нужно вставить ниже
    var spreadsheetUrl = "https://docs.google.com/spreadsheets/d/XXXXXXXXXXXXX";

    var minNGramLength = 1;
    var maxNGramLength = 5;
    // Минимальная и максимальная длинна для анализа поисковых запросов
        ...
        for (var campaign in adGroupNGrams) {
            for (var adGroup in adGroupNGrams[campaign]) {
                for (var nGram in adGroupNGrams[campaign][adGroup][n]) {
                    ...
                    if (adGroupNGrams[campaign][adGroup][n][nGram]["Conversions"] < conversionThreshold) {
                        continue;
                    }
    ...
    for (var n = minNGramLength; n < maxNGramLength + 1; n++) {
        for (var campaign in campaignNGrams) {
            for (var nGram in campaignNGrams[campaign][n]) {
                ...
                if (campaignNGrams[campaign][n][nGram]["Cost"] < costThreshold) {
                    continue;
                }
    ...
    for (var n = minNGramLength; n < maxNGramLength + 1; n++) {
        for (var nGram in totalNGrams[n]) {
            ...
            if (totalNGrams[n][nGram]["Cost"] < costThreshold) {
                continue;
            }
            if (totalNGrams[n][nGram]["Conversions"] < conversionThreshold) {
                continue;
            }
...
function writeOutput(outputArray, formatArray, sheet, nGramName, levelName, filterText, clearSpreadsheet) {
    for (var i = 0; i < 5; i++) {
        try {
            ...
            sheet.getRange("R2C2").setValue('Account currency = ' + AdWordsApp.currentAccount().getCurrencyCode());
