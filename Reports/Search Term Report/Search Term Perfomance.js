 ...
 *
 *
 * Скрипт Google Ads, который показывает эффективность поисковых запросов, которые определяет специалист по контекстной рекламе.
 *
...
function main() {
    ...
    // Установить периоды анализа
    // Формат yyyy-mm-dd

    var currencySymbol = "₴";
    // Поставить релевантный символ для валюты. Пример "£", "$" or "€", "₴", "₽".

    var campaignNameDoesNotContain = [];
    // Используйте если, нужно исключить кампании
    // Пример ["Display"] исключит все РК с 'Display' в названии,
    // Оставьте пустым [], если ничего не нужно исключать.

    var campaignNameContains = [];
    // Используйте если, нужно включить определенные кампании
    // Пример ["Display"] включит все РК с 'Display' в названии,
    // Оставьте пустым [], если нужно включить все кампании.

    var ignorePausedCampaigns = true;
    // Оставить true, если нужно отработать только включеныне кампании

    var ignorePausedAdGroups = true;
    // Оставить true, если нужно отработать только включеныне группы


    var spreadsheetUrl = "https://docs.google.com/YOUR-SPREADSHEET-URL-HERE";
    // Поставьте целевой URL на Google Sheet.

    var adjectives = ["example one", "example two"];
    // Впишите поисковые запросы по которым нужно провести анализ

    var clearSpreadsheet = true;
    // Если значение true, данные, находящиеся в электронной таблице, будут перезаписаны.

    var lookAtCompletePhrases = true;
    // Используйте true, если нужен отчет только по точным фразам
